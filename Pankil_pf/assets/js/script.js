...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set default active button and list
  toggleButtons[0].classList.add("active");
  skillsList.classList.add("active");

  const handleTransition = (addClass, removeClass) => {
    addClass.classList.add("active");
    addClass.classList.remove("inactive");
    removeClass.classList.add("inactive");
  ...
  toggleButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (button.textContent === "Skills") {
        handleTransition(skillsList, toolsList);
      } else {
        handleTransition(toolsList, skillsList);
      }
    });
  });
});
